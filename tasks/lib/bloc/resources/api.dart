...
import 'dart:convert';
import 'package:tasks/models/classes/user.dart';
...
class ApiProvider {
  ...
  Future<User> registerUser(String username, String firstname, String lastname,
    ...
    final response =
        await client.post("http://127.0.0.1:5000/api/register" as Uri,
            // headers: "",
            body: jsonEncode({
              "emailadress": email,
              "username": username,
  ...
  Future signinUser(String username, String password, String apiKey) async {
    final response =
        await client.post("http://127.0.0.1:5000/api/signin" as Uri,
  ...
  Future<List<Task>> getUserTasks(String apiKey) async {
    final response = await client.get(
      "http://127.0.0.1:5000/api/tasks" as Uri,
      headers: {"Authorization": apiKey},
    );
    final Map result = json.decode(response.body);
    if (response.statusCode == 201) {
  ...
  Future addUserTask(String apiKey, String taskName, String deadline) async {
    final response = await client.post("http://127.0.0.1:5000/api/tasks" as Uri,
        headers: {"Authorization": apiKey},
        body: jsonEncode({
          "note": "",
          "repeats": "",
